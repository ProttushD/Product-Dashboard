...
function loadPlans(){
    ...
    if(savedPlans){
        const loadedPlans = JSON.parse(savedPlans);

        loadedPlans.forEach((savedSlot)=>{

            const originalSlot = timings.find((slot)=>{
                return slot.id === savedSlot.id;
            });

            if(originalSlot){
                originalSlot.plan = savedSlot.plan;
            }
